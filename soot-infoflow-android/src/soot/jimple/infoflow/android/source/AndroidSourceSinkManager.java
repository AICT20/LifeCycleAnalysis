...
public class AndroidSourceSinkManager implements ISourceSinkManager, IOneSourceAtATimeManager {
	...
	private Map<Stmt, SinkInfo> exitSPMap = null;
	@Override
	public SinkInfo getSPSinkInfo(Stmt returnSite, InfoflowManager manager, String tag){
		if (tag.equals("exit")) {
			SinkInfo sink = exitSPMap.get(returnSite);
			if (null == sink) {
				SourceSinkDefinition def = new ExitSourceSinkDefinition(returnSite);
				sink = new SinkInfo(def);
				exitSPMap.put(returnSite, sink);
			}
			return sink;
		}
		return null;
