...
public class IFDSSolver<N, D extends FastSolverLinkedNode<D, N>, I extends BiDiInterproceduralCFG<N, SootMethod>>
	 ...
	 */
	protected void scheduleEdgeProcessing(PathEdge<N, D> edge, N previousN) {
...
//		new PathEdgeProcessingTask(edge, solverId).run();
	...
	private void processCall(PathEdge<N, D> edge) {
					...
					if (res != null && !res.isEmpty()) {
						...
						for (D d3 : res) {
							...
							for (N sP : startPointsOf) {
								...
								propagate(d3, sP, d3, n, false, n); // line 15
							}
		...
		for (N returnSiteN : returnSiteNs) {
			FlowFunction<D> callToReturnFlowFunction = flowFunctions.getCallToReturnFlowFunction(n, returnSiteN);//这里的flowFuntions是InfoflowProblem,返回的是SolverCallToReturnFlowFunction
			...
			if (res != null && !res.isEmpty()) {
				for (D d3 : res) {
					if (memoryManager != null)
						d3 = memoryManager.handleGeneratedMemoryObject(d2, d3);
					if (d3 != null)
						propagate(d1, returnSiteN, d3, n, false, n);
				}
			}
		}
	}
				...
				for (N retSiteN : returnSiteNs) {
					...
					if (retFlowRes != null && !retFlowRes.isEmpty()) {
						...
						for (D d5 : retFlowRes) {
							...
							propagate(d1, retSiteN, d5p, n, false, n);
						}
					}
				}
			}
					...
					if (targets != null && !targets.isEmpty()) {
						for (Entry<D, D> d1d2entry : entry.getValue().entrySet()) {
							...
							for (D d5 : targets) {
								...
								propagate(d4, retSiteC, d5p, c, false, n);
							}
						}
					}
				}
					...
					if (targets != null && !targets.isEmpty()) {
						for (D d5 : targets) {
							if (memoryManager != null)
								d5 = memoryManager.handleGeneratedMemoryObject(d2, d5);
							if (d5 != null)
								propagate(zeroValue, retSiteC, d5, c, true, n);
						}
					}
				}
			}
			...
			if (res != null && !res.isEmpty()) {
				for (D d3 : res) {
					if (memoryManager != null && d2 != d3)
						d3 = memoryManager.handleGeneratedMemoryObject(d2, d3);
					if (d3 != null)
						propagate(d1, m, d3, null, false, n);
				}
			}
		}
	}
	...
	protected void propagate(D sourceVal, N target, D targetVal,
			...
			/* deliberately exposed to clients */ boolean isUnbalancedReturn, N previousN) {
	...
	private static Set<Unit> previousNsForTest = new HashSet<>();

	private class PathEdgeProcessingTask implements Runnable {

		private final PathEdge<N, D> edge;
		private final boolean solverId;
		private final N previousN;

		public PathEdgeProcessingTask(PathEdge<N, D> edge, boolean solverId, N previousN) {
			this.edge = edge;
			this.solverId = solverId;
