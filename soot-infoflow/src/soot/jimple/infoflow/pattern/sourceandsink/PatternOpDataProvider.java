...
public class PatternOpDataProvider {
    ...
    protected class SAXHandler extends DefaultHandler {
        ...
        protected LCResourceOPType opType = null;//如EDIT, RELEASE等等最终输出的形式，也是判断路径是否有问题的依据
        protected Set<LCMethodDefinition> methodDefs = null;//一个definition可能包含多个method，而每个method只能转为同一个definition
        protected MergeStrategy mergeStrategy = MergeStrategy.NEVER;//用于在加入这个def生成的op时，对oplist应当如何操作；
        public SAXHandler() {
            super();
            this.opdefs.add(LCEditOPDefinition.getSingletonEditDefinition());//把单独的edit加进去
        }
        @Override
        ...
        public void endElement(String uri, String localName, String qName) throws SAXException {
            ...
            switch (qNameLower) {
                case LCResourceOPConstant.xml_operation: {
                    //加入opdefs
                    opdefs.add(new LCResourceOPDefinition(category, opType, methodDefs, mergeStrategy));
                    break;
                }
            }
        }
